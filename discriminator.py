...
class Discriminator(nn.Module):
  def __init__(self):
    ...
    self.linear_cond = nn.Linear(40, 256*6*6, bias=False)
  ...
  def forward(self, x, cond):
    ...
    cond = self.linear_cond(cond.float())
    return x + torch.sum(cond*x_flatten, dim=1, keepdim=True)
...
def discriminator_loss(pred_synth, pred_real):
  ...
  return F.binary_cross_entropy_with_logits(pred_synth, t_synth) + F.binary_cross_entropy_with_logits(pred_real, t_real)